...
Boss-level v2 (NO EPS): PDF + PNG + JPG

Fixes vs v1 (based on your annotated output):
1) Panel C y-labels were cropped on the left:
   - increase figure left margin
   - reduce ytick label fontsize slightly + add tick padding
   - add extra internal x-margin in panel C
2) Panel E legend (WT/Mut) was too close / visually crowded:
   - move legend slightly inward + tighter handlelength
3) RuntimeWarning (All-NaN axis encountered) removed:
   - safe min-fragment computation when both WT+Mut missing

Truth source: thesis-derived assay_table.csv (expected fragment sizes).
"""
...
PAL = {
    ...
    # Premium palette (distinct from Fig3–5)
...
def main():
    ...
    for _, r in df.iterrows():
        ...
        minsp_wt.append(ms_wt); minsp_mut.append(ms_mut)
        minfrag_wt.append(mf_wt); minfrag_mut.append(mf_mut)

        # Safe min fragment (prevents All-NaN warning)
        cand = []
    ...
    axB.text(tau, axB.get_ylim()[1]*0.92, f"τ={tau:g} bp", fontsize=8, color=PAL["risk_high"], ha="center")
    ...
    w = 0.35
    axE.bar(x - w/2, wt_vals, width=w, color=PAL["wt"], edgecolor=PAL["midnight"], linewidth=0.4, label="WT")
    axE.bar(x + w/2, mut_vals, width=w, color=PAL["mut"], edgecolor=PAL["midnight"], linewidth=0.4, label="Mut")
    axE.set_xticks(x)
    ...
    axE.legend(frameon=False, fontsize=9, loc="upper right",
               bbox_to_anchor=(0.98, 0.98), handlelength=1.2, borderaxespad=0.2)
    axE.text(0.01, -0.22, f"WT total={part_wt['total']}  •  Mut total={part_mut['total']}",
             transform=axE.transAxes, fontsize=8, color=PAL["slate"], ha="left", va="top")
    ...
    # Increased LEFT margin to prevent y-label cropping in Panel C
    fig.subplots_adjust(left=0.070, right=0.975, top=0.965, bottom=0.065)
