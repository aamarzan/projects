...
"""
figure_1_prisma_goat_v6.py

Premium PRISMA figure generator (journal-ready aesthetics).

What changed vs v5 (fixing the issues you pointed out):
- Title made professional: removes "2020" from the title (PRISMA can be cited in caption instead).
- Removes the "Tip:" line entirely.
- Left spine boxes are no longer overly wide (tighter width; better proportionality).
- Right-side box text is CENTERED (horizontal + vertical) for a cleaner look.
- Bottom 3 panels are resized to be proportional to the amount of text they contain.
- Prevents text overflow: slightly taller key boxes + safer wrapping widths + consistent padding.
- More consistent geometry: uniform elbow connectors (no awkward long curves).

Run:
  python figure_1_prisma_goat_v6.py --write_template prisma_counts.json
  # edit prisma_counts.json
  python figure_1_prisma_goat_v6.py --counts prisma_counts.json --outdir figures --bottom_panel --docx "Multiple Myeloma.docx" --eps

Outputs: PNG (DPI), PDF, SVG (and EPS optional).
"""
...
def connect_down(ax, upper_box, lower_box, color="#262626", lw=1.45, ms=12.5):
    ...
    m = min(0.07, max(0.03, gap * 0.30)) if gap > 0 else 0.04
    y0 = upper_bottom - m
    y1 = lower_top + m
    if y0 <= y1:
        y0 = upper_bottom - 0.03
        y1 = lower_top + 0.03
...
def draw_prisma(ax, c: PrismaCounts, subtitle: Optional[str] = None):
    ax.set_axis_off()
    ax.set_xlim(0, 21.2)
    ax.set_ylim(0, 10)
    ...
    # title
    label(ax, 10.6, 9.74, "PRISMA flow diagram", size=16, weight="bold")
    if subtitle:
        label(ax, 10.6, 9.38, subtitle, size=10.8, color="#444444")

    # --- Layout (tighter, more proportional) ---
    xM, wM = 1.55, 8.95     # main column narrower than v5
    xS, wS = 11.25, 8.20    # right column
    h = 0.90
    h_tall = 1.05           # for text-heavy boxes
    dy = 1.18
    y0 = 8.28
    ...
    # stage labels
    label(ax, 0.75, b_ident[1] + b_ident[3] / 2,  "Identification", ha="right", size=10.6, weight="bold", color="#2b4d8a")
    label(ax, 0.75, b_screen[1] + h / 2,          "Screening",      ha="right", size=10.6, weight="bold", color="#8a5a00")
    label(ax, 0.75, b_assess[1] + h / 2,          "Eligibility",    ha="right", size=10.6, weight="bold", color="#1f6b3b")
    label(ax, 0.75, b_meta[1] + h / 2,            "Included",       ha="right", size=10.6, weight="bold", color="#7a1f4c")

    # ---- Text (wrapped to avoid overflow) ----
    ident_txt = (
    ...
    # center the whole block as requested
    label(ax, b_removed[0] + wS / 2, b_removed[1] + b_removed[3] / 2, wrap_keep_newlines(removed_txt, 46), ha="center", va="center", size=9.0)
    ...
    # center this panel too (as per your rule for right-side boxes)
    label(
        ax,
        b_excl_ft[0] + wS / 2,
        b_excl_ft[1] + b_excl_ft[3] / 2,
        wrap_keep_newlines(ft_txt, 52),
        ha="center",
        va="center",
        size=9.0,
    )
    ...
    # ---- Side connectors (consistent elbow) ----
    ...
    arrow(ax, (xM + wM + 0.12, b_screen[1] + h / 2), (xS - 0.10, b_excl_ta[1] + h / 2),
          color=arrowc, lw=1.25, ms=12.0, connectionstyle=elbow)
    arrow(ax, (xM + wM + 0.12, b_sought[1] + h / 2), (xS - 0.10, b_notret[1] + h / 2),
          color=arrowc, lw=1.25, ms=12.0, connectionstyle=elbow)
...
def draw_bottom_panel(ax, c: PrismaCounts, bd_summary: Optional[Dict[str, Any]]):
    ...
    # widths proportional to typical text volume (snapshot is widest)
    x0 = 1.55
    y = 0.42
    h = 2.05
    gap = 0.75
    w1, w2, w3 = 6.40, 6.05, 5.70
    ...
    label(ax, x0 + 0.35, y + h - 0.65, wrap_keep_newlines("\n".join(lines), 58), ha="left", va="top", size=8.8, color="#222222")
    ...
    if bd_summary:
        ...
        body = top + "\nDesigns: " + design_str
        label(ax, x1 + 0.35, y + h - 0.65, wrap_keep_newlines(body, 54), ha="left", va="top", size=8.7, color="#222222")
    else:
    ...
    label(ax, x2 + 0.35, y + h - 0.65, wrap_keep_newlines(diag, 52), ha="left", va="top", size=8.7, color="#333333")
...
def main():
    ...
    ap.add_argument("--basename", type=str, default="Figure_1_PRISMA_GOAT_v6")
    ...
    save_kws = dict(bbox_inches="tight", facecolor="white", pad_inches=0.12)
