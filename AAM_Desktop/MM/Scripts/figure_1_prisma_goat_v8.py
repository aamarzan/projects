...
def draw_bottom_panel(ax, c: PrismaCounts, bd_summary: Optional[Dict[str, Any]], layout: Layout,
                      bottom_gap: float = 0.85, bottom_ratios: Tuple[float, float, float] = (1.10, 1.00, 1.00)):
    ax.set_axis_off()
    ax.set_xlim(0, layout.xmax)
    ax.set_ylim(0, 2.85)
    border = "#1f1f1f"
...
def main():
    ...
    ap.add_argument("--main_w", type=float, default=7)
    ap.add_argument("--side_w", type=float, default=7)
    ap.add_argument("--col_gap", type=float, default=2.0)

    ap.add_argument("--bottom_gap", type=float, default=0.85)
    ap.add_argument("--bottom_ratios", type=str, default="1.10,1.00,1.00")
