#!/usr/bin/env python3
"""
Figure 1 (Premium v5.3): Clean workflow schematic with reordered lanes.
- In-silico row moved to the first/top section.
- Wet-lab row moved to the second/middle section.
- Verification & Reporting remains the third/bottom section.
...
FINAL_FIG_DIR = PROJECT_ROOT / "Final Figure"
...
def draw_vertical_gradient(ax, x, y, w, h, c_top, c_bottom, steps=90, z=3):
    ...
    for i in range(steps):
        ...
        g = lerp(gt, gb, t)
        b = lerp(bt, bb, t)
        ax.add_patch(Rectangle((x, y + i * dh), w, dh, facecolor=(r, g, b), edgecolor="none", zorder=z))
...
def add_step(ax, x, y, w, h, title, body,
    ...
    ax.plot([x + 0.002, x + w - 0.002], [y + h - 0.002, y + h - 0.002], color="#FFFFFF", lw=0.7, zorder=4)
    ...
    b = wrap_for_box(body, w_axes=w) if wrap_body else body
    body_text = ax.text(
        x + 0.014, y + (header_y - y) / 2,
        b, ha="left", va="center",
        fontsize=body_fs, color="#111827",
        clip_on=True, zorder=5
    )
...
def build(out_dir: Path, stem="Figure_1_Workflow_reordered"):
    apply_style()
    ...
    # Lane backgrounds reordered with the content blocks.
    ax.add_patch(Rectangle((0.02, 0.64), 0.96, 0.20, facecolor="#F1FDFF", edgecolor="none", zorder=0))
    ax.add_patch(Rectangle((0.02, 0.36), 0.96, 0.20, facecolor="#F0FFFD", edgecolor="none", zorder=0))
    ...
    ax.text(0.02, 0.84, "In-silico (Assay design & expected patterns)", ha="left", va="bottom",
            fontsize=12, weight="bold", color="#0B1220", zorder=20)
    ax.text(0.02, 0.56, "Wet-lab (Sample → PCR–RFLP)", ha="left", va="bottom",
            fontsize=12, weight="bold", color="#0B1220", zorder=20)
    ...
    # -------- Row 1 (In-silico) --------
    y1, h1 = 0.67, 0.15
    xs1, w1 = row_positions(4, left=0.04, right=0.02, gap=0.04)

    i1 = add_step(ax, xs1[0], y1, w1, h1, "Reference & target selection",
    ...
    # -------- Row 2 (Wet-lab) --------
    y2, h2 = 0.39, 0.15
    xs2, w2 = row_positions(5, left=0.04, right=0.02, gap=0.03)

    s1 = add_step(ax, xs2[0], y2, w2, h2, "Clinical specimen",
                  "Nasopharyngeal swab\n(or equivalent)",
                  grad_top=wet_grads[0][0], grad_bottom=wet_grads[0][1])
    s2 = add_step(ax, xs2[1], y2, w2, h2, "RNA extraction + QC",
                  "Yield/quality\ncheck; accept or repeat",
                  grad_top=wet_grads[1][0], grad_bottom=wet_grads[1][1])
    s3 = add_step(ax, xs2[2], y2, w2, h2, "RT-qPCR screening",
                  "Ct thresholding;\nselect positives",
                  grad_top=wet_grads[2][0], grad_bottom=wet_grads[2][1])
    s4 = add_step(ax, xs2[3], y2, w2, h2, "cDNA synthesis → PCR",
                  "Amplify mutation\nlocus panel (25 loci)",
                  grad_top=wet_grads[3][0], grad_bottom=wet_grads[3][1],
                  wrap_title=False, title_fs=9.0)
    s5 = add_step(ax, xs2[4], y2, w2, h2, "RFLP digestion + gel",
                  "Banding pattern\ncomparison (mutant vs wild)",
    ...
    v2 = add_step(ax, xs3[1], y3, w3, h3, "Consensus mutation call",
                  ...
                  wrap_body=False, body_fs=8.0)
    v3 = add_step(ax, xs3[2], y3, w3, h3, "Standardized reporting outputs",
                  "Panel map; signature atlas; QC\noutputs; representative gel check",
    ...
    # Restriction mapping -> RFLP digestion + gel
    y_corridor_12 = (y2 + h2 + y1) / 2
    x_i3 = mid_bottom(i3)[0]
    p_end_s5 = (s5.get_x() + s5.get_width() * 0.82, s5.get_y() + s5.get_height())
    x_end = p_end_s5[0]

    routed_arrow(
        ax,
        [mid_bottom(i3), (x_i3, y_corridor_12), (x_end, y_corridor_12), p_end_s5],
        color="#0F766E",
        lw=1.15, ms=12, z=12,
    ...
    ax.text(
        (x_i3 + (s5.get_x() + s5.get_width() / 2)) / 2,
        y_corridor_12 + 0.020,
    ...
    # RFLP digestion + gel -> Consensus mutation call
    ...
    routed_arrow(
        ax,
        [p_start, (x_margin, y_start), (x_margin, y_corridor_23), (x_v2t, y_corridor_23), mid_top(v2)],
        color="#7C2D12",
        lw=1.15, ms=12, z=12,
    ...
    save_figure_pdf_a4(fig, base.with_suffix(".pdf"), facecolor="white", landscape=True, margin_mm=12.0)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", default=str(FINAL_FIG_DIR / "Fig_1"))
