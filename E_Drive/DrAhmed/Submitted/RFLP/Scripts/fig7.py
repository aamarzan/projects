...
Other panels unchanged. Input source: finalized assay definition table (expected fragment sizes).
"""
...
KNOWN_PROJECT_ROOTS = [
    Path(r"E:\DrAhmed\Ongoing\RFLP"),
    Path(r"E:\Dr. Ahmed\Ongoing\RFLP"),
]
...
def candidate_project_roots():
    ...
    if SCRIPT_DIR.name.lower() == "scripts":
        guesses.append(SCRIPT_DIR.parent)
    guesses.extend([
        SCRIPT_DIR,
        SCRIPT_DIR.parent,
        *KNOWN_PROJECT_ROOTS,
    ])
    return _unique_paths(guesses)

def resolve_assay_csv_path(path: str) -> str:
    candidates = []
    if path:
        candidates.append(Path(path))
    for root in candidate_project_roots():
        candidates.extend([
            root / "Scripts" / "inputs" / "assay_table.csv",
            root / "inputs" / "assay_table.csv",
        ])
    checked = []
    ...
    raise FileNotFoundError(
        "Could not find assay_table.csv. Checked these locations:\n- " + "\n- ".join(checked)
    )

def resolve_output_dir(path: str, fig_name: str) -> str:
    if path and str(path).strip():
        return str(Path(path))
    for root in candidate_project_roots():
        parent = root / "Final Figure"
        try:
            parent.mkdir(parents=True, exist_ok=True)
            return str(parent / fig_name)
        except Exception:
            continue
    return str((SCRIPT_DIR / fig_name).resolve())

DEFAULT_ASSAY_CSV = ""
DEFAULT_GEL3 = ""
WINDOWS_GEL3_FALLBACK = Path(r"E:\Dr. Ahmed\Ongoing\RFLP\PDF_extract\Thesis Figures\f79.png")
WINDOWS_GEL3_FALLBACK_2 = Path(r"E:\DrAhmed\Ongoing\RFLP\PDF_extract\Thesis Figures\f79.png")
...
def resolve_gel_inset(path: str) -> Path | None:
    candidates = []
    if path:
        candidates.append(Path(path))
    for root in candidate_project_roots():
        candidates.append(root / "PDF_extract" / "Thesis Figures" / "f79.png")
    candidates.extend([WINDOWS_GEL3_FALLBACK, WINDOWS_GEL3_FALLBACK_2])
...
def main():
    ...
    ap.add_argument("--assay_csv", default=DEFAULT_ASSAY_CSV)
    ...
    ap.add_argument("--gel_inset", default=DEFAULT_GEL3,
                    help="Optional path to a cleaned gel image (PNG/JPG). Falls back to thesis gel if omitted.")
    args = ap.parse_args()

    assay_csv = resolve_assay_csv_path(args.assay_csv)
    args.outdir = resolve_output_dir(args.outdir, "Fig_7")
    ...
    gel_path = resolve_gel_inset(args.gel_inset)
    ...
    fig.text(
        0.02, 0.02,
        "Figure 7 is implementation-focused: operational calling workflow and QC gates derived from expected fragment signatures. "
        "Protocol inputs (gel percentage and ladder) should match the study methods.",
