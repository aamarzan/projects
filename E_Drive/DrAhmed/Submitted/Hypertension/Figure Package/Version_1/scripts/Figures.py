...
def write_report():
    ...
    txt = '''# Figure Generation Notes

This script generates the figure files used for manuscript preparation.

## Figure 1
- Cohort derivation workflow showing screening, exclusions, follow-up windows, and the final analytic cohort.

## Figure 2
- Panel A: PH subtype composition across baseline risk categories.
- Panel B: transition in ESC/ERS risk categories from T0 to T4 with ordinal severity summary.
- Panel C: NT-proBNP distribution across visits on a log10 scale.
- Panel D: six-minute walk distance distribution across visits.
- Panel E: haemoglobin distribution across visits.

## Figure 3
- Panel A: overall composite event-free survival.
- Panel B: event-free survival stratified by baseline ESC/ERS risk category, with number-at-risk table.

## Data note
Where individual-level observations were not available, distributional displays were generated from the reported summary statistics. Final inferential figures should be regenerated from the source patient-level dataset when available.
'''
    (REP/'figure_generation_notes.md').write_text(txt)
...
def main():
    ...
    figure3_survival()
    write_report()
    (ROOT/'README.txt').write_text('Figure generation script. Outputs are written to the figures directory as PNG and PDF files.\n')
