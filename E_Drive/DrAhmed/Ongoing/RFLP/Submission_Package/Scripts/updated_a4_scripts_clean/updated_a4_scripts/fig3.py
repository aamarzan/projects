...
- Corrected site-effect counting (gain/loss/ambiguous parsed from conditional site-effect strings such as "gain|none").
...
Inputs:
  --assay_csv : finalized assay definition table
...
DEFAULT_ASSAY_CSV = INPUTS_DIR / "assay_table.csv"
DEFAULT_GEL3 = PROJECT_ROOT / "PDF_extract" / "Thesis Figures" / "f79.png"
...
def resolve_gel_path(path: Optional[str]) -> str:
    candidates = []
    if path:
        candidates.append(Path(path))
    candidates.extend([DEFAULT_GEL3, WINDOWS_GEL3_FALLBACK])
...
def load_gel(path: str):
    resolved = resolve_gel_path(path)
    if not resolved:
        return None
    if Image is None:
        return None
    try:
        im = Image.open(resolved).convert("RGB")  # EPS-safe (no alpha)
...
def make_figure(assay_csv: str, gel_path: str, outdir: str, title: str):
    ...
    gel_path = resolve_gel_path(gel_path)
    ...
    # Footer note
    fig.text(0.02, 0.012,
            "Panel B synthetic lanes are computed from expected fragment sizes in the finalized assay definition table. "
            "Ambiguity (*) denotes alternative mutant outcomes retained for conditional loci.",
    ...
    save_figure_pdf_a4(fig, pdf, facecolor="white", landscape=True, margin_mm=12.0,
                       native_savefig_kwargs={"bbox_inches": "tight"})
...
def main():
    ...
    ap.add_argument("--assay_csv", default=str(DEFAULT_ASSAY_CSV), help="Path to assay_table.csv")
    ap.add_argument("--gel", default=str(DEFAULT_GEL3 if DEFAULT_GEL3.exists() else WINDOWS_GEL3_FALLBACK),
                    help="Path to ONE representative burnished gel image (png/jpg). Falls back to thesis gel if omitted.")
    ap.add_argument("--outdir", default=str(FINAL_FIG_DIR / "Fig_3"),
                    help="Output directory (e.g., E:/.../Final Figure/Fig_3)")
