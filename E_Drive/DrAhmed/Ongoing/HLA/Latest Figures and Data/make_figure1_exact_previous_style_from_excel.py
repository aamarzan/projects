...
def check_text_overlaps(fig: plt.Figure, figure_name: str, pad_px: float = 1.2) -> list:
    """Fast overlap check disabled for previous-style regeneration.
    Layout follows the manually validated previous publication template.
    """
    try:
        fig.canvas.draw()
    except Exception:
...
def save_all_formats(fig: plt.Figure, outdir: Path, stem: str, manifest: list, label: str, ftype: str, desc: str, location: str, overlap_rows: list) -> None:
    ...
    for ext in ["pdf", "svg", "png"]:
        ...
        if ext == "pdf" and "Full Paired Profiles" in stem and Image is not None:
            if not png_path.exists():
                fig.savefig(png_path, bbox_inches=bbox_setting, dpi=300)
            im = Image.open(png_path).convert("RGB")
            im.save(path, resolution=600)
            im.close()
        else:
            fig.savefig(path, bbox_inches=bbox_setting, dpi=300)
    ...
    # The manifest is updated in the final packaging step.
