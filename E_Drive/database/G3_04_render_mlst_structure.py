...
from G3_02_plotting_helper import (
    ...
    save_png_pdf, style_ax, palette_list, ensure_dir
)
...
# keep meaningful species groups
species_order = []
tmp = defaultdict(int)
for r in rows:
    tmp[norm(r["TopSpecies"])] += as_int(r["Count"])
for sp, n in sorted(tmp.items(), key=lambda x: (-x[1], x[0])):
    if n >= 2:
        species_order.append(sp)

# if a species has many STs, keep top 12 and collapse rest to Other
panel_data = {}
...
save_png_pdf(fig, "Figure03_MLST_Structure_G3", OUT)

print("Rendered:", f"{OUT}/Figure03_MLST_Structure_G3.png")
