...
def major_species_from_counts(rows, min_n=4):
    spcol = guess_col(rows, ["TopSpecies", "Species"])
    ctcol = guess_col(rows, ["Count", "Samples_n", "N"])
    out = []
    for r in rows:
        sp = norm(r.get(spcol, ""))
        n = as_int(r.get(ctcol, 0))
        if sp and n >= min_n:
            out.append((sp, n))
...
def heatmap_from_long(ax, rows, row_order, row_col, key_col, val_col,
    ...
    ax.set_xticklabels([wrap(k, 16) for k in keys], rotation=35, ha="right")
