...
High-resolution PNG, PDF, and SVG files saved to:
    E:\Zafrul_Sir\RBG\Update\Figures\HighImpact_10of10
...
def savefig(fig: plt.Figure, stem: str) -> None:
    for ext in ("png", "pdf", "svg"):
        fig.savefig(OUT_DIR / f"{stem}.{ext}", bbox_inches="tight", pad_inches=0.18, dpi=DPI, facecolor=WHITE)
    plt.close(fig)


def add_panel_label(ax: plt.Axes, label: str) -> None:
    """Panel label placed inside axes in a small navy tag to avoid title overlap."""
    ax.text(0.012, 0.985, label, transform=ax.transAxes, ha="left", va="top",
            fontsize=16, fontweight="bold", color=WHITE, zorder=20,
            bbox=dict(boxstyle="round,pad=0.22,rounding_size=0.08", fc=NAVY, ec=NAVY, lw=0.0, alpha=0.96))
...
def fig1_complete_case_validity(raw: pd.DataFrame, sub: pd.DataFrame, col: Dict[str, str]) -> None:
    ...
    gs = gridspec.GridSpec(2, 3, figure=fig, height_ratios=[1.0, 1.12], wspace=0.40, hspace=0.30)
    ...
    boxes = [
        ...
        (0.03, 0.40, 0.44, 0.17, f"Observed accident history\nn={observed:,} ({observed/total*100:.1f}%)"),
        (0.53, 0.40, 0.44, 0.17, f"Missing accident history\nn={missing:,} ({missing/total*100:.1f}%)"),
        (0.03, 0.12, 0.44, 0.17, f"No accident\nn={no:,}"),
        (0.53, 0.12, 0.44, 0.17, f"Accident history\nn={yes:,}"),
    ]
    ...
    ax_missing.set_yticks(np.arange(len(md))); ax_missing.set_yticklabels(md["Stratum"], fontsize=10.0)
    ...
    fig.suptitle("Figure 1. Complete-case validity and accident-outcome architecture", fontsize=18, fontweight="bold", color=NAVY, y=0.965)
    fig.text(0.01, 0.01, "Purpose: documents outcome incompleteness, observed accident burden, and whether missing accident history may bias inference.", color=GREY, fontsize=11)
...
def fig2_accident_phenotype(sub: pd.DataFrame) -> None:
    ...
    gs = gridspec.GridSpec(2, 3, figure=fig, wspace=0.40, hspace=0.30)
    ...
    for i in range(len(labels)):
        for j in range(len(labels)):
            ax_corr.text(j, i, f"{corr.iloc[i,j]:.2f}", ha="center", va="center", fontsize=9.5,
    ...
    ax_density.legend(frameon=False, fontsize=10.0)
    ...
    fig.suptitle("Figure 2. Multidimensional accident phenotype and separability", fontsize=18, fontweight="bold", color=NAVY, y=0.965)
    fig.text(0.01, 0.01, "Purpose: identifies whether accident-history cases form a distinct phenotype across workload, cumulative exposure, metabolic profile, and correlation structure.", color=GREY, fontsize=11)
    fig.subplots_adjust(top=0.88, bottom=0.09, left=0.08, right=0.97)
...
def fig3_adjusted_inference(model, model_df: pd.DataFrame) -> None:
    ...
    ax_heat.clabel(cs, inline=True, fontsize=10.5, fmt="%.1f%%")
    # Overlay points only inside the modelled quantile window so the surface fills the panel.
    inside = model_df["RBG"].between(rbg_grid.min(), rbg_grid.max()) & model_df["Driving_H_D"].between(dh_grid.min(), dh_grid.max())
    ax_heat.scatter(model_df.loc[inside, "RBG"], model_df.loc[inside, "Driving_H_D"],
                    c=model_df.loc[inside, "accident"],
                    cmap=mpl.colors.ListedColormap(["#FFFFFF55", "#00FFFFBB"]),
                    s=14, alpha=0.50, edgecolor="none")
    ax_heat.set_xlim(rbg_grid.min(), rbg_grid.max())
    ax_heat.set_ylim(dh_grid.min(), dh_grid.max())
    ...
    if not boot.empty:
        ...
        ax_boot.set_yticks(positions); ax_boot.set_yticklabels([readable_term(t) for t in order], fontsize=10.0)
    ...
    fig.suptitle("Figure 3. Adjusted inference, uncertainty, and nonlinear risk surface", fontsize=18, fontweight="bold", color=NAVY, y=0.965)
    fig.text(0.01, 0.01, "Purpose: moves beyond OR tables by showing adjusted magnitude, uncertainty, marginal absolute risk, joint risk surface, and bootstrap coefficient stability.", color=GREY, fontsize=11)
    fig.subplots_adjust(top=0.88, bottom=0.09, left=0.08, right=0.97)
...
def fig4_performance(model_df: pd.DataFrame, pred: np.ndarray) -> None:
    ...
    fig.suptitle("Figure 4. Model discrimination, calibration, and decision value", fontsize=18, fontweight="bold", color=NAVY, y=0.965)
    fig.text(0.01, 0.01, "Purpose: evaluates whether the adjusted model is not only statistically significant but also predictive, calibrated, and useful across practical risk thresholds.", color=GREY, fontsize=11)
    fig.subplots_adjust(top=0.88, bottom=0.09, left=0.08, right=0.97)
...
def fig5_actionable_translation(model, model_df: pd.DataFrame, sub: pd.DataFrame) -> None:
    ...
    ax_nom.set_yticks(y); ax_nom.set_yticklabels(tab["label"], fontsize=10.0)
    ...
    for cutoff in [20, 30, 50]:
        ...
        ax_obs.text(cutoff, ev+3, f"Top {cutoff}%\ncaptures {ev:.0f}%", ha="center", fontsize=10.0)
        ...
        for j in range(matrix.shape[1]):
            ...
            if len(row):
                ...
                ax_joint.text(j, i, f"{matrix.iloc[i,j]:.1f}%\n{int(r.events)}/{int(r.n)}", ha="center", va="center",
                              fontsize=10, fontweight="bold", color=INK if matrix.iloc[i,j] >= 45 else WHITE)
    ...
    for i, r in top.iterrows():
        ...
        ax_rank.text(r["pred"]*100 + 1.0, i, txt, va="center", fontsize=9.5, clip_on=False)
    ax_rank.set_xlim(0, max(80, top["pred"].max()*100 + 18))
    ax_rank.grid(axis="x", alpha=0.35)

    fig.suptitle("Figure 5. Actionable risk stratification and occupational-health translation", fontsize=18, fontweight="bold", color=NAVY, y=0.965)
    fig.text(0.01, 0.01, "Purpose: translates the adjusted model into interpretable risk ranking, workload–glycemia strata, and prevention-oriented prioritization.", color=GREY, fontsize=11)
    fig.subplots_adjust(top=0.88, bottom=0.09, left=0.08, right=0.97)
...
def supplementary_unadjusted_or_atlas() -> None:
    ...
    fig, ax = plt.subplots(figsize=(14, max(9, 0.56*len(df))), facecolor=WHITE)
    y = np.arange(len(df))
    ...
    ax.set_xscale("log")
    ax.set_yticks(y); ax.set_yticklabels(df["label"], fontsize=10.0)
    ...
    fig.subplots_adjust(top=0.90, bottom=0.08, left=0.30, right=0.97)
