...
# -*- coding: utf-8 -*-
r"""
V14 collision-safe publication figure generator for the 874-driver accident-history manuscript.
...
Default output:
    E:\Zafrul_Sir\RBG\Update\Figures\v14_collision_safe_master

Recommended local final run:
    python E:\Zafrul_Sir\RBG\Update\Scripts\Accident_Figure_v14_Collision_Safe_Master.py --version v14_collision_safe_master --dpi 600 --bootstrap 100 --strict-vector
...
DEFAULT_VERSION = "v14_collision_safe_master"
...
def plot_estimate_card_columns(
    ...
    for yi, (_, r) in zip(y, dd.iterrows()):
        ax_plot.hlines(yi, r["lo"], r["hi"], color=COLORS["slate"], lw=1.05, zorder=2)
    ...
    ax_plot.scatter(dd["OR"], y, s=(25 if compact else 30), color=marker_colors,
    ...
    ax_table.text(0.0, -0.72, header, ha="left", va="center",
                  fontsize=6.85, fontweight="bold", color=COLORS["slate"], clip_on=False)
    for yi, (_, r) in zip(y, dd.iterrows()):
        ax_table.text(0.0, yi, f"{r['OR']:.2f} ({r['lo']:.2f}-{r['hi']:.2f}); {fmt_p(r.get('p', np.nan))}",
                      ha="left", va="center", fontsize=6.75 if compact else 6.9, color=COLORS["slate"], clip_on=True)
...
def plot_forest_with_side_table(ax: plt.Axes, ax_tbl: plt.Axes, d: pd.DataFrame, title: str,
    ...
    ax_tbl.text(0.0, -0.72, header, ha="left", va="center", fontsize=6.85, fontweight="bold", color=COLORS["slate"], clip_on=False)
    for yi, (_, r) in zip(y, dd.iterrows()):
        ax_tbl.text(0.0, yi, f"{r['OR']:.2f} ({r['lo']:.2f}-{r['hi']:.2f}); {fmt_p(r.get('p', np.nan))}",
                    ha="left", va="center", fontsize=6.9, color=COLORS["slate"], clip_on=True)
...
def plot_domain_architecture(ax: plt.Axes, screening: pd.DataFrame) -> None:
    ...
    for sx, (_, col) in enumerate(stages):
        for yi, (_, r) in enumerate(dom.iterrows()):
            ...
            ax.text(sx, yi, f"{val}", ha="center", va="center", fontsize=6.8,
    ...
    ax.set_yticks(y)
    ax.set_yticklabels(dom["domain"], fontsize=7.2)
...
def plot_prevalence_contrast_columns(
    ...
    ax_label.set_xlim(0, 1); ax_label.axis("off")
    ax_plot.set_title(title, pad=7)
    for endpoint in endpoints:
        ax_label.text(0.98, y_positions[endpoint], endpoint, ha="right", va="center",
                      fontsize=7.2, fontweight="bold", color=colors[endpoint], clip_on=True)

    ax_plot.set_xlim(0, 72)
    ax_plot.set_xlabel("Observed prevalence (%)")
    ax_plot.set_yticks([])
    ...
    for endpoint in endpoints:
        ...
        ax_plot.text(vals[0], y0-0.19, "New", ha="center", va="top", fontsize=5.8, color=COLORS["muted"])
        ax_plot.text(vals[1], y0+0.19, "Renew", ha="center", va="bottom", fontsize=5.8, color=COLORS["muted"])

    ax_value.set_xlim(0, 1); ax_value.axis("off")
    ax_value.text(0.0, 1.42, "%; n/N", ha="left", va="center", fontsize=6.7, fontweight="bold", color=COLORS["slate"])
    ax_delta.set_xlim(0, 1); ax_delta.axis("off")
    ax_delta.text(0.0, 1.42, "Change", ha="left", va="center", fontsize=6.7, fontweight="bold", color=COLORS["slate"])
    for endpoint in endpoints:
        dd = prev[prev["endpoint"].eq(endpoint)].set_index("license").loc[["New license", "Renew license"]]
        y0 = y_positions[endpoint]
        values_text = (
        ...
        ax_value.text(0.0, y0, values_text, ha="left", va="center", fontsize=6.05, color=COLORS["slate"], linespacing=1.20, clip_on=True)
        delta = float(dd.iloc[1]["prev_pct"] - dd.iloc[0]["prev_pct"])
        ax_delta.text(0.02, y0, f"+{delta:.1f} pp", ha="left", va="center", fontsize=7.0,
                      fontweight="bold", color=colors[endpoint], clip_on=True)
...
def build_figure_1(df: pd.DataFrame, sheets: dict[str, pd.DataFrame], paths: RunPaths, variant: str, include_title: bool, dpi: int, records: list, qc: list):
    ...
    fig = plt.figure(figsize=(12.1, 7.9), constrained_layout=False)
    gs = fig.add_gridspec(2, 3, width_ratios=[1.20, 1.02, 1.65], height_ratios=[1, 1.08], wspace=0.40, hspace=0.58)
    axA = fig.add_subplot(gs[0,0]); axB = fig.add_subplot(gs[0,1])

    sgC = gs[0,2].subgridspec(1, 4, width_ratios=[0.34, 0.055, 0.33, 0.275], wspace=0.035)
    axCl = fig.add_subplot(sgC[0,0]); axCb = fig.add_subplot(sgC[0,1]); axCp = fig.add_subplot(sgC[0,2]); axCt = fig.add_subplot(sgC[0,3])

    axD = fig.add_subplot(gs[1,0])
    sgE = gs[1,1:].subgridspec(1, 4, width_ratios=[0.30, 0.045, 0.40, 0.255], wspace=0.035)
    axEl = fig.add_subplot(sgE[0,0]); axEb = fig.add_subplot(sgE[0,1]); axEp = fig.add_subplot(sgE[0,2]); axEt = fig.add_subplot(sgE[0,3])

    add_panel_label(axA,"A"); plot_evidence_skyline(axA, screening, 16, "Candidate-signal landscape", "-log10(p)", 18, show_legend=False)
    add_panel_label(axB,"B"); plot_domain_architecture(axB, screening)
    add_panel_label(axCl,"C")
    plot_estimate_card_columns(axCl, axCb, axCp, axCt, core[["label","OR","lo","hi","p"]], "Adjusted signal estimates", xlim=(0.85,3.9), label_size=6.95, compact=True)
    ...
    add_panel_label(axEl,"E")
    dcat = cat[(cat["OR"]>1) & (cat["p"]<0.01) & (cat["n_category"]>=20)].copy().sort_values("p").head(8)
    plot_estimate_card_columns(axEl, axEb, axEp, axEt, dcat[["label","OR","lo","hi","p"]], "Categorical signal estimates", xlim=(0.85,6.8), label_size=6.7, compact=True)

    legend_handles = [Line2D([0],[0], marker='o', color='none', markerfacecolor=v, markeredgecolor=COLORS['white'], markersize=5.0, label=DOMAIN_SHORT[k]) for k,v in DOMAIN_COLORS.items()]
    legend_handles += [Line2D([0],[0], marker='o', color=COLORS['slate'], markerfacecolor=COLORS['white'], markersize=5.0, label='Nominal'),
                       Line2D([0],[0], marker='o', color='none', markerfacecolor=COLORS['navy'], markersize=5.0, label='FDR q<=0.10')]
    fig.legend(handles=legend_handles, frameon=False, ncol=4, loc='lower left', bbox_to_anchor=(0.085, 0.017), fontsize=6.4)
    add_figure_title(fig, "Candidate-signal atlas and adjusted accident-history estimates", include_title)
    fig.subplots_adjust(left=0.080, right=0.985, top=0.91 if include_title else 0.975, bottom=0.108)
    records.extend(save_figure_all_formats(fig, "Figure_1_Candidate_Signal_Atlas", "main", variant, paths, dpi))
    if variant == "titled_review": qc.append("Figure 1: collision-safe four-column estimate systems used for adjusted and categorical estimates; labels, badges, intervals, and numeric estimates occupy separate axes.")
...
def build_figure_2(df: pd.DataFrame, sheets: dict[str, pd.DataFrame], paths: RunPaths, variant: str, include_title: bool, dpi: int, records: list, qc: list):
    fig = plt.figure(figsize=(9.5,6.8), constrained_layout=False)
    gs = fig.add_gridspec(2,2,wspace=0.38,hspace=0.52)

    sgA = gs[0,0].subgridspec(1, 4, width_ratios=[0.23, 0.39, 0.27, 0.11], wspace=0.035)
    ...
    add_panel_label(axAl,"A"); plot_prevalence_contrast_columns(axAl, axAp, axAv, axAd, prev, "Observed prevalence contrasts")
    ...
    sgC = gs[1,0].subgridspec(1,2,width_ratios=[0.62,0.38],wspace=0.055)
    ...
    fig.subplots_adjust(left=0.09,right=0.98,top=0.91 if include_title else 0.97,bottom=0.10)
    records.extend(save_figure_all_formats(fig, "Figure_2_License_Betel_Clustering", "main", variant, paths, dpi))
    if variant == "titled_review": qc.append("Figure 2: Panel A converted to a four-column prevalence contrast card with reserved label, plot, value, and change columns; no +pp badges use edge/negative coordinates.")
...
def build_figure_3(df: pd.DataFrame, sheets: dict[str, pd.DataFrame], paths: RunPaths, variant: str, include_title: bool, dpi: int, records: list, qc: list) -> dict[str,float]:
    ...
    sgA=gs[0,0].subgridspec(1,4,width_ratios=[0.34,0.055,0.33,0.275],wspace=0.035)
    ...
    add_panel_label(axAl,"A")
    plot_estimate_card_columns(axAl, axAb, axA, axAt, action[["label","OR","lo","hi","p"]], "Adjusted signal estimates", xlim=(0.85,3.9), label_size=6.95, compact=True)
    ...
    axD.clabel(cs,inline=True,fontsize=5.9,fmt="%.1f%%")
...
def plot_domain_leading_cards(ax: plt.Axes, screening: pd.DataFrame) -> None:
    ...
    for idx, (_, r) in enumerate(best.iterrows()):
        ...
        ax.text(x0+0.075, y0+h-0.12, dom, ha="left", va="top", fontsize=7.0, fontweight="bold", color=col)
        ax.text(x0+0.075, y0+h-0.34, fill(var, 12), ha="left", va="top", fontsize=6.2, color=COLORS["ink"], linespacing=1.05)
        ax.text(x0+w-0.055, y0+0.165, f"{float(r['strength']):.1f}", ha="right", va="bottom", fontsize=7.0, fontweight="bold", color=COLORS["slate"])
        ax.text(x0+w-0.055, y0+0.065, "-log10(p)", ha="right", va="bottom", fontsize=5.0, color=COLORS["muted"])
        ax.text(x0+0.075, y0+0.10, marker, ha="left", va="bottom", fontsize=5.8, color=COLORS["muted"])
...
def build_supp_s1(df:pd.DataFrame,sheets:dict[str,pd.DataFrame],paths:RunPaths,variant:str,include_title:bool,dpi:int,records:list,qc:list):
    ...
    add_panel_label(axA,"A"); plot_domain_leading_cards(axA, screening)
...
def write_final_submission_readiness(paths: RunPaths, records: list[dict[str, str]]) -> Path:
    ...
    lines.append("- The v14 collision-safe package is the recommended programmatic figure set for manuscript-draft integration.")
    ...
    lines.append("- Supplementary Figure S4: 8.6/10 - useful internal performance support; keep supplementary unless model utility becomes central.")
    lines.append("\n## Export readiness")
    ...
    lines.append("- Figure 4 title/marginal-bar collision: addressed by explicit reserved layout bands; v14 also adds automated renderer collision reporting.")
...
def write_qc_report(paths: RunPaths, df: pd.DataFrame, sheets: dict[str,pd.DataFrame], qc:list, metrics:dict[str,float], validation_metrics:dict[str,float], dpi:int, manifest_path:Path, contact_sheet:Path, inventory_path:Path, checklist_path:Path) -> Path:
    ...
    lines=[]; lines.append("# v14 collision-safe figure QC report\n")
    ...
    lines.append("\n## v14 collision-safe design changes\n")
...
def parse_args(argv: Optional[Sequence[str]]=None) -> argparse.Namespace:
    p=argparse.ArgumentParser(description="Generate v12 submission-master figure package for accident-history manuscript.")
...
def main(argv: Optional[Sequence[str]]=None) -> int:
    ...
    manifest=write_manifest(paths,records); contact_png, contact_pdf=create_contact_sheets(paths,records); collision_report=write_collision_report(paths); checklist=write_visual_checklist(paths, records, args.dpi); vector_report=write_vector_export_report(paths, records); change_log=write_change_log(paths); readiness=write_final_submission_readiness(paths, records); report=write_qc_report(paths,df,sheets,qc,metrics,validation_metrics,args.dpi,manifest,contact_pdf,inventory_path,checklist)
