...
def plot_estimate_card_columns(
    ...
    for yi, (_, r) in zip(y, dd.iterrows()):
        ax_table.text(0.0, yi, f"{r['OR']:.2f} ({r['lo']:.2f}-{r['hi']:.2f}); {fmt_p(r.get('p', np.nan))}",
                      ha="left", va="center", fontsize=6.25 if compact else 6.7, color=COLORS["slate"], clip_on=False)
...
def plot_domain_architecture_safe(ax_label: plt.Axes, ax_strip: plt.Axes, ax_matrix: plt.Axes, screening: pd.DataFrame) -> None:
    ...
    dom = pd.DataFrame(rows).sort_values("screened", ascending=True).reset_index(drop=True)
    stages = [("Screened", "screened"), ("p<0.05", "nominal"), ("FDR q≤0.10", "fdr")]
    stage_x = np.array([0.0, 1.18, 2.48])
    n = len(dom)
    ...
    ax_label.axis("off")
    for yi, (_, r) in enumerate(dom.iterrows()):
        ax_label.text(0.98, yi, r["domain"], ha="right", va="center", fontsize=7.0, color=COLORS["ink"], clip_on=True)
    ...
    ax_matrix.set_title("Domain-level signal architecture", pad=6, loc="center")
    max_by_stage = {col: max(1, int(dom[col].max())) for _, col in stages}
    for sx, (_, col) in zip(stage_x, stages):
        for yi, (_, r) in enumerate(dom.iterrows()):
            val = int(r[col])
            ...
            ax_matrix.add_patch(FancyBboxPatch((sx - 0.42, yi - 0.34), 0.84, 0.68,
                                               boxstyle="round,pad=0.012,rounding_size=0.035",
                                               facecolor=fc, edgecolor=COLORS["white"], lw=0.9, zorder=2))
            ax_matrix.text(sx, yi, f"{val}", ha="center", va="center", fontsize=6.8,
                           color=COLORS["white"] if alpha > 0.55 else COLORS["ink"],
                           fontweight="bold", zorder=3)
        ax_matrix.axvline(sx + 0.59, color=COLORS["grid"], lw=0.45, zorder=1)
    for yi in range(n):
        ax_matrix.axhline(yi + 0.5, color=COLORS["grid"], lw=0.45, zorder=1)
    ax_matrix.set_xticks(stage_x)
    ax_matrix.set_xticklabels([s[0] for s in stages], fontsize=6.4)
    ax_matrix.tick_params(axis="x", pad=2)
    ax_matrix.set_xlim(-0.55, 2.98)
...
def plot_prevalence_contrast_columns(
    ...
    for endpoint in endpoints:
        ax_label.text(0.94, y_positions[endpoint], endpoint, ha="right", va="center",
                    fontsize=7.2, fontweight="bold", color=colors[endpoint],
                    clip_on=False)
    ...
    ax_value.text(0.0, 1.42, "% ; n/N".replace(" ", ""), ha="left", va="center", fontsize=6.7, fontweight="bold", color=COLORS["slate"])
    ...
    for endpoint in endpoints:
        ...
        ax_value.text(0.0, y0, values_text, ha="left", va="center", fontsize=5.85, color=COLORS["slate"], linespacing=1.20, clip_on=False)
        delta = float(dd.iloc[1]["prev_pct"] - dd.iloc[0]["prev_pct"])
        y_change = y0 - 0.10 if endpoint == "Betel quid" else y0
        ax_delta.text(0.02, y_change, f"+{delta:.1f} pp", ha="left", va="center", fontsize=6.6,
...
def build_figure_1(df: pd.DataFrame, sheets: dict[str, pd.DataFrame], paths: RunPaths, variant: str, include_title: bool, dpi: int, records: list, qc: list):
    ...
    sgB = gs[0,1].subgridspec(1, 3, width_ratios=[0.35, 0.055, 0.595], wspace=0.030)
    axBl = fig.add_subplot(sgB[0,0]); axBs = fig.add_subplot(sgB[0,1]); axBm = fig.add_subplot(sgB[0,2])

    sgC = gs[0,2].subgridspec(1, 4, width_ratios=[0.30, 0.045, 0.285, 0.370], wspace=0.035)
    ...
    add_panel_label(axBl,"B"); plot_domain_architecture_safe(axBl, axBs, axBm, screening)
    ...
    plot_estimate_card_columns(axEl, axEb, axEp, axEt, dcat[["label","OR","lo","hi","p"]], "Categorical signal estimates", xlim=(0.85,6.8), label_size=6.7, compact=True, or_gradient=True)
...
def build_figure_2(df: pd.DataFrame, sheets: dict[str, pd.DataFrame], paths: RunPaths, variant: str, include_title: bool, dpi: int, records: list, qc: list):
    ...
    sgA = gs[0,0].subgridspec(1, 4, width_ratios=[0.30, 0.36, 0.24, 0.10], wspace=0.050)
...
def build_figure_3(df: pd.DataFrame, sheets: dict[str, pd.DataFrame], paths: RunPaths, variant: str, include_title: bool, dpi: int, records: list, qc: list) -> dict[str,float]:
    ...
    sgA=gs[0,0].subgridspec(1,4,width_ratios=[0.30,0.045,0.285,0.370],wspace=0.035)
...
def build_figure_4(df: pd.DataFrame, sheets: dict[str,pd.DataFrame], paths: RunPaths, variant: str, include_title: bool, dpi:int, records:list, qc:list):
    ...
    fig.text(0.18 + 0.54/2, 0.875, "Observed burden by field-measurable strata",
             ha="center", va="center", fontsize=9.4, fontweight="bold", color=COLORS["ink"])
    ...
    fig.text(left + width, 0.098, f"† n<{SPARSE_N_THRESHOLD}; intended for population-level discussion, not individual prediction rule",
...
def plot_domain_leading_cards(ax: plt.Axes, screening: pd.DataFrame) -> None:
    ...
    for idx, (_, r) in enumerate(best.iterrows()):
        ...
        ax.text(x0+0.075, y0+h-0.34, safe_fill(var, 16), ha="left", va="top", fontsize=6.2, color=COLORS["ink"], linespacing=1.05)
