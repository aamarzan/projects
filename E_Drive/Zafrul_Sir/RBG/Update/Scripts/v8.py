...
# -*- coding: utf-8 -*-
r"""
V8 final publication figure generator for the 874-driver accident-history manuscript.
...
Default output:
    E:\Zafrul_Sir\RBG\Update\Figures\v8_final

Recommended local final run:
    python E:\Zafrul_Sir\RBG\Update\Scripts\Accident_Figure_v8_Final.py --version v8_final --dpi 600 --bootstrap 100 --strict-vector
...
def plot_median_contrast(ax: plt.Axes, df: pd.DataFrame, variables: list[str], title: str,
    ...
    rows = []
    for v in variables:
        rows.append(bootstrap_median_difference(df, v, n_boot=n_boot))
    dd = pd.DataFrame(rows).dropna(subset=["diff"]).sort_values("diff")
    y = np.arange(len(dd))
    shade_alternate_rows(ax, len(dd))
    ax.axvline(0, color=COLORS["muted"], lw=0.8)
    ax.hlines(y, dd["lo"], dd["hi"], color=COLORS["slate"], lw=1.0, zorder=2)
    color_list = []
    for v in dd["variable"]:
        if "Driving" in v:
            color_list.append(COLORS["occupational"])
        elif "RBG" in v:
            color_list.append(COLORS["metabolic"])
        elif "Quid" in v or "C_N" in v or "SFood" in v:
            color_list.append(COLORS["lifestyle"])
        else:
            color_list.append(COLORS["demographic"])
    ax.scatter(dd["diff"], y, s=26, color=color_list, edgecolor=COLORS["white"], lw=0.55, zorder=3)
    ax.set_yticks(y)
...
def save_figure_all_formats(fig: plt.Figure, stem: str, category: str, variant: str, paths: RunPaths, dpi: int) -> list[dict[str, str]]:
    ...
    if all(p.exists() and p.stat().st_size > 0 for p in expected.values()):
        for fmt, path in expected.items():
            records.append({"category": category, "variant": variant, "format": fmt, "file": str(path), "bytes": str(path.stat().st_size), "skipped_existing": "true"})
    ...
    records.append({"category": category, "variant": variant, "format": "png", "file": str(png), "bytes": str(png.stat().st_size)})
    with Image.open(png).convert("RGB") as img:
        jpg = dirs["jpeg"] / f"{stem}.jpg"
        img.save(jpg, "JPEG", quality=95, optimize=True, dpi=(dpi, dpi))
        records.append({"category": category, "variant": variant, "format": "jpeg", "file": str(jpg), "bytes": str(jpg.stat().st_size)})
        tif = dirs["tiff"] / f"{stem}.tiff"
        img.save(tif, "TIFF", compression="tiff_lzw", dpi=(dpi, dpi))
        records.append({"category": category, "variant": variant, "format": "tiff", "file": str(tif), "bytes": str(tif.stat().st_size)})
    # Direct vector PDF where feasible. SVG is direct only when --strict-vector is
    # requested; otherwise it uses a high-resolution wrapper to keep package
    # generation stable in constrained environments. EPS uses a documented raster
    # fallback because the PostScript backend is slow and does not support alpha
    # transparency reliably for this full package.
    for fmt in (["pdf", "svg"] if STRICT_VECTOR_OUTPUT else ["pdf"]):
        out = dirs[fmt] / f"{stem}.{fmt}"
        try:
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                fig.savefig(out, format=fmt, dpi=dpi, **common)
            records.append({"category": category, "variant": variant, "format": fmt, "file": str(out), "bytes": str(out.stat().st_size)})
        except Exception:
            with Image.open(png).convert("RGB") as img:
                if fmt == "pdf":
            ...
            records.append({"category": category, "variant": variant, "format": fmt, "file": str(out), "bytes": str(out.stat().st_size), "fallback": "raster_wrapper"})
    if not STRICT_VECTOR_OUTPUT:
        ...
        records.append({"category": category, "variant": variant, "format": "svg", "file": str(svg), "bytes": str(svg.stat().st_size), "fallback": "raster_wrapper"})
    eps = dirs["eps"] / f"{stem}.eps"
    with Image.open(png).convert("RGB") as img:
        img.save(eps, "EPS")
    records.append({"category": category, "variant": variant, "format": "eps", "file": str(eps), "bytes": str(eps.stat().st_size), "fallback": "raster_eps"})
    plt.close(fig); plt.close("all"); gc.collect()
    return records
...
def write_manifest(paths: RunPaths, records: list[dict[str, str]]) -> Path:
    ...
    fieldnames = ["category", "variant", "format", "file", "bytes", "fallback"]
...
def build_figure_2(df: pd.DataFrame, sheets: dict[str, pd.DataFrame], paths: RunPaths, variant: str, include_title: bool, dpi: int, records: list, qc: list):
    ...
    sm,_=plot_tile_burden(axB, joint, "license_group", "betel_group", ["New license","Renew license"], ["No betel","Betel"], "Joint license-betel burden", CMAPS["license_betel"], with_sample_circles=True)
    ...
    plot_forest_with_side_table(axC, axCt, pd.DataFrame(rows), "Adjustment attenuation profile", xlim=(0.8,6.5), label_size=7.0)
...
def build_figure_3(df: pd.DataFrame, sheets: dict[str, pd.DataFrame], paths: RunPaths, variant: str, include_title: bool, dpi: int, records: list, qc: list) -> dict[str,float]:
    ...
    # Gradient strip behind line encodes predicted prevalence.
    grad = np.tile(np.linspace(0,1,len(rg)), (2,1))
    axB.imshow(grad, extent=[rg.min(), rg.max(), 0, 17], origin="lower", cmap=mpl.colors.LinearSegmentedColormap.from_list("rbg_grad", ["#FFFFFF", COLORS["light_rbg"]]), aspect="auto", alpha=0.55, zorder=0)
...
def build_figure_4(df: pd.DataFrame, sheets: dict[str,pd.DataFrame], paths: RunPaths, variant: str, include_title: bool, dpi:int, records:list, qc:list):
    ...
    sm,_=plot_tile_burden(ax,hm,"Drive_cat","RBG_cat",row_order,col_order,panel_title,CMAPS["matrix"],with_sample_circles=True)
...
def build_supp_s3(df:pd.DataFrame,sheets:dict[str,pd.DataFrame],paths:RunPaths,variant:str,include_title:bool,dpi:int,records:list,qc:list):
    screening=prepare_screening(sheets); top=screening.dropna(subset=["p_value"]).sort_values("p_value").head(30).copy()
    fig,ax=plt.subplots(figsize=(7.4,8.6)); add_panel_label(ax,"A"); plot_evidence_skyline(ax,top,30,"Candidate-variable evidence ranking","Association strength, -log10(p)",24,show_legend=True)
    add_figure_title(fig,"Full candidate-variable discovery screen",include_title); fig.subplots_adjust(left=.30,right=.98,top=.91 if include_title else .98,bottom=.08)
    ...
    if variant=="titled_review": qc.append("Supplementary Figure S3: full discovery screen retains top 30 variables with significance zone shading and FDR/open-marker distinction.")
...
def write_visual_checklist(paths: RunPaths, records: list[dict[str, str]], dpi: int) -> Path:
    ...
    for c in checks:
        lines.append(f"- [x] {c}")
    lines.append(f"\nRaster DPI for this run: {dpi}. Local final production should use --dpi 600.")
...
def write_qc_report(paths: RunPaths, df: pd.DataFrame, sheets: dict[str,pd.DataFrame], qc:list, metrics:dict[str,float], validation_metrics:dict[str,float], dpi:int, manifest_path:Path, contact_sheet:Path, inventory_path:Path, checklist_path:Path) -> Path:
    ...
    lines=[]; lines.append("# v8 final figure QC report\n")
    ...
    lines.append(f"- Strict vector mode requested by script: {STRICT_VECTOR_OUTPUT}")
...
def parse_args(argv: Optional[Sequence[str]]=None) -> argparse.Namespace:
    p=argparse.ArgumentParser(description="Generate v8 final figure package for accident-history manuscript.")
...
def main(argv: Optional[Sequence[str]]=None) -> int:
    ...
    manifest=write_manifest(paths,records); contact=create_contact_sheet(paths,records); checklist=write_visual_checklist(paths, records, args.dpi); report=write_qc_report(paths,df,sheets,qc,metrics,validation_metrics,args.dpi,manifest,contact,inventory_path,checklist)
