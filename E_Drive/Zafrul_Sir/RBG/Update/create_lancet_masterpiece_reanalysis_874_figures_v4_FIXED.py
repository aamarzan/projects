...
# -*- coding: utf-8 -*-
r"""
Final fixed Lancet-style figure generator for the 874-driver accident-history re-analysis
=======================================================================================
...
Outputs:
    Figures\Reanalysis_874_Lancet_Masterpiece_v4_FIXED
"""
...
OUT_DIR = PROJECT_DIR / "Figures" / "Reanalysis_874_Lancet_Masterpiece_v4_FIXED"
...
def savefig(fig: plt.Figure, stem: str) -> None:
    ...
    fig.savefig(png_path, dpi=DPI, bbox_inches="tight", pad_inches=0.24, facecolor=WHITE)
    try:
        img = Image.open(png_path).convert("RGB")
        img.save(pdf_path, "PDF", resolution=DPI)
    except Exception:
        fig.savefig(pdf_path, dpi=DPI, bbox_inches="tight", pad_inches=0.24, facecolor=WHITE)
    if SAVE_SVG:
        fig.savefig(OUT_DIR / f"{stem}.svg", dpi=DPI, bbox_inches="tight", pad_inches=0.24, facecolor=WHITE)
    plt.close(fig)
    plt.close("all")
    gc.collect()
...
def forest(ax: plt.Axes, d: pd.DataFrame, title: str, xlim: Optional[Tuple[float,float]] = None,
    ...
    if annotate:
        ...
        for yi, (_, r) in enumerate(d.iterrows()):
            xtext = min(max(r["hi"]*1.10, r["OR"]*1.20), xmax/1.16)
            ax.text(xtext, yi, f"{r['OR']:.2f}, {fmt_p(r.get('p', np.nan))}",
                    va="center", ha="left", fontsize=9.4, color=SLATE,
...
def figure1_discovery(df, sheets):
    ...
    gs = gridspec.GridSpec(2, 3, figure=fig, width_ratios=[1.18, 1.0, 1.06], height_ratios=[1.05, 1.0], wspace=0.42, hspace=0.45)
...
def figure2_license_betel(df, sheets):
    ...
    fig.subplots_adjust(left=0.065,right=0.98,top=0.91,bottom=0.075)
    savefig(fig,"Figure_2_License_Betel_Pathway")


def figure3_smoking_betel_rbg(df, sheets):
    fig = plt.figure(figsize=(23.5, 15.5), facecolor=WHITE)
    gs = gridspec.GridSpec(2, 3, figure=fig, width_ratios=[1,1,1], height_ratios=[1,1], wspace=0.40, hspace=0.45)
    axA=fig.add_subplot(gs[0,0]); axB=fig.add_subplot(gs[0,1]); axC=fig.add_subplot(gs[0,2])
    axD=fig.add_subplot(gs[1,0]); axE=fig.add_subplot(gs[1,1]); axF=fig.add_subplot(gs[1,2])
    ...
    for j,(m,lab,c) in enumerate([(m1,"Group only",CRIMSON),(m2,"Group + RBG",BLUE)]):
        ...
        for xi, yi, pp in zip(vals, y+(j-0.5)*0.18, ps):
            axD.text(xi*1.06, yi, f"{xi:.2f}, {fmt_p(pp)}", fontsize=9.4, va="center", ha="left", color=SLATE,
                     bbox=dict(boxstyle="round,pad=0.12", fc=WHITE, ec="none", alpha=0.80))
    rbg_or=math.exp(m2.params["RBG_num"]); rbg_p=m2.pvalues["RBG_num"]
    axD.text(0.02,0.04,f"RBG in pathway model: OR {rbg_or:.2f} per mmol/L, {fmt_p(rbg_p)}", transform=axD.transAxes, fontsize=10.8,
             bbox=dict(boxstyle="round,pad=0.28", fc=WHITE, ec=GRID))
    axD.set_xscale("log"); axD.set_xlim(0.6,6.0)
