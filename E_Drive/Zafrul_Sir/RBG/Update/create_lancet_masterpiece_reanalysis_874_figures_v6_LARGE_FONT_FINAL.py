...
# -*- coding: utf-8 -*-
r"""
Final large-font Lancet-style figure generator for the 874-driver accident-history re-analysis
=======================================================================================
...
Run on Windows:
    ...
    python .\create_lancet_masterpiece_reanalysis_874_figures_v6_LARGE_FONT_FINAL.py
...
Outputs:
    Figures\Reanalysis_874_Lancet_Masterpiece_v6_LARGE_FONT_FINAL
"""
...
OUT_DIR = PROJECT_DIR / "Figures" / "Reanalysis_874_Lancet_Masterpiece_v6_LARGE_FONT_FINAL"
...
def savefig(fig: plt.Figure, stem: str) -> None:
    """Save large-font figure as high-resolution PNG.

    PDF export is intentionally skipped in this large-font review version to
    prevent slow image-based PDF conversion. The PNGs are manuscript-quality and
    can be inserted into Word/PowerPoint directly.
    """
    png_path = OUT_DIR / f"{stem}.png"
    fig.savefig(png_path, dpi=DPI, bbox_inches=None, facecolor=WHITE)
    if SAVE_SVG:
        fig.savefig(OUT_DIR / f"{stem}.svg", dpi=DPI, bbox_inches=None, facecolor=WHITE)
    plt.close(fig)
    plt.close("all")
    gc.collect()
...
def add_panel_label(ax: plt.Axes, label: str) -> None:
    ...
    ax.text(0.012, 0.988, label, transform=ax.transAxes, ha="left", va="top",
            fontsize=18.5, fontweight="bold", color=WHITE, zorder=50,
...
def title_block(fig: plt.Figure, title: str, subtitle: str = "") -> None:
    fig.text(0.5, 0.986, title, ha="center", va="top", fontsize=24.0, fontweight="bold", color=NAVY)
    if subtitle:
        fig.text(0.5, 0.958, subtitle, ha="center", va="top", fontsize=14.5, color=MUTED)


def footer(fig: plt.Figure, text: str) -> None:
    fig.text(0.012, 0.014, text, ha="left", va="bottom", fontsize=12.8, color=MUTED)
    ...
    if annotate:
        ...
        for yi, (_, r) in enumerate(d.iterrows()):
            ax.text(0.985, yi, f"{r['OR']:.2f}  {fmt_p(r.get('p', np.nan))}",
                    ...
                    fontsize=10.8, color=SLATE, clip_on=True,
...
def figure1_discovery(df, sheets):
    ...
    fig = plt.figure(figsize=(34.0, 22.5), facecolor=WHITE)
    gs = gridspec.GridSpec(2, 3, figure=fig, width_ratios=[1.18, 1.0, 1.06], height_ratios=[1.05, 1.0], wspace=0.62, hspace=0.66)
    ...
    axA.legend(handles=handles, ncol=3, loc="lower right", frameon=False, fontsize=14.6)
...
def figure2_license_betel(df, sheets):
    fig = plt.figure(figsize=(34.0, 22.5), facecolor=WHITE)
    gs = gridspec.GridSpec(2, 3, figure=fig, width_ratios=[1.10,1.04,1.04], height_ratios=[1,1], wspace=0.52, hspace=0.58)
    ...
    axA.text(0.10,0.40, f"Renew → betel: OR {o1.OR:.2f}\nRenew → accident: OR {o2.OR:.2f}\nAfter adding betel: OR {o3.OR:.2f}", fontsize=13.9, color=INK,
             bbox=dict(boxstyle="round,pad=0.35", fc=WHITE, ec=GRID))
    axA.text(0.07,0.18, fill("Meaning: betel quid is related to both license status and accident history, but it does not fully explain the license signal.", 62), fontsize=14.2, color=SLATE,
    ...
    for i,r in summ.iterrows():
        ...
        axB.text(idx-width/2, r["betel_percent"]+2, f"{r['betel_percent']:.1f}%", ha="center", fontsize=12.8, fontweight="bold")
        axB.text(idx+width/2, r["accident_percent"]+2, f"{r['accident_percent']:.1f}%", ha="center", fontsize=12.8, fontweight="bold")
    ...
    for i,(num,txt,col) in enumerate(cards):
        ...
        axF.text(0.115,y0+0.058-i*0.18,num,ha="center",va="center",fontsize=14.6,fontweight="bold",color=WHITE)
        axF.text(0.22,y0+0.058-i*0.18,fill(txt,34),ha="left",va="center",fontsize=13.2,color=INK)
    axF.text(0.05,0.05,"Reviewer-safe wording:\nBetel quid partly co-occurs with renew-license status,\nbut the license–accident signal remains after betel adjustment.", fontsize=14.2, color=SLATE,
             bbox=dict(boxstyle="round,pad=0.35", fc=WHITE, ec=GRID))
...
def figure3_smoking_betel_rbg(df, sheets):
    fig = plt.figure(figsize=(34.0, 22.5), facecolor=WHITE)
    gs = gridspec.GridSpec(2, 3, figure=fig, width_ratios=[1,1,1], height_ratios=[1,1], wspace=0.60, hspace=0.66)
    axA=fig.add_subplot(gs[0,0]); axB=fig.add_subplot(gs[0,1]); axC=fig.add_subplot(gs[0,2])
    axD=fig.add_subplot(gs[1,0]); axE=fig.add_subplot(gs[1,1]); axF=fig.add_subplot(gs[1,2])
    ...
    for i,r in summ.iterrows(): axA.text(i, r["prev"]+1.4, f"{r['prev']:.1f}%\n{int(r.events)}/{int(r.n)}", ha="center", fontsize=12.8, fontweight="bold")
    ...
    axB.text(0.97,0.93,f"RBG group difference\n{fmt_p(p)}", transform=axB.transAxes, ha="right", va="top", fontsize=12.8,
    ...
    for j,(m,lab,c) in enumerate([(m1,"Group only",CRIMSON),(m2,"Group + RBG",BLUE)]):
        ...
        for xi, yi, pp in zip(vals, y+(j-0.5)*0.18, ps):
            axD.text(min(xi*1.06,7.55), yi, f"{xi:.2f} {fmt_p(pp)}", fontsize=10.8, va="center", ha="left", color=SLATE,
...
def figure4_core_model(df, sheets):
    fig = plt.figure(figsize=(31.5,20.5), facecolor=WHITE)
    gs = gridspec.GridSpec(2,3,figure=fig,width_ratios=[1.05,1,1],height_ratios=[1,1.05],wspace=0.54,hspace=0.60)
    axA=fig.add_subplot(gs[0,0]); axB=fig.add_subplot(gs[0,1]); axC=fig.add_subplot(gs[0,2])
    ...
    axD.clabel(cs,inline=True,fontsize=14.6,fmt="%.1f%%")
    ...
    for cutoff in [20,30,50]:
        ...
        axE.text(cutoff+2,ev-3,f"Top {cutoff}%\n{ev:.0f}% cases",fontsize=14.6,color=INK,bbox=dict(boxstyle="round,pad=0.18",fc=WHITE,ec="none",alpha=0.78))
    fpr,tpr,_=roc_curve(y,pred)
    axE.text(0.05,0.96,f"AUC {auc(fpr,tpr):.3f}\nAverage precision {average_precision_score(y,pred):.3f}\nBrier {brier_score_loss(y,pred):.3f}",transform=axE.transAxes,ha="left",va="top",fontsize=12.2,bbox=dict(boxstyle="round,pad=0.25",fc=WHITE,ec=GRID))
...
def figure5_domain_atlas(df, sheets):
    ...
    screening["strength"]=-np.log10(screening["p_value"].clip(lower=1e-300))
    fig=plt.figure(figsize=(31.5,20.5),facecolor=WHITE)
    gs=gridspec.GridSpec(2,3,figure=fig,width_ratios=[1,1.05,1.05],height_ratios=[1,1],wspace=0.58,hspace=0.62)
    axA=fig.add_subplot(gs[0,0]); axB=fig.add_subplot(gs[0,1]); axC=fig.add_subplot(gs[0,2])
    axD=fig.add_subplot(gs[1,0]); axE=fig.add_subplot(gs[1,1]); axF=fig.add_subplot(gs[1,2])
    ...
    for yi,(_,r) in enumerate(cont.iterrows()):
        axD.text(r["diff"]+0.15 if r["diff"]>=0 else r["diff"]-0.15, yi, fmt_p(r["p_value"]), va="center", ha="left" if r["diff"]>=0 else "right", fontsize=10.8, color=SLATE)
    ...
    for i,(title,body,fill_col,edge_col) in enumerate(cards):
        ...
        axF.text(0.10, yy+0.082, title, ha="left", va="center", fontsize=12.8,
                 fontweight="bold", color=edge_col)
        axF.text(0.10, yy+0.038, fill(body, 44), ha="left", va="center", fontsize=14.6,
                 color=INK)
...
def figure6_dashboard(df, sheets):
    ...
    for i,(title,val,col) in enumerate(cards):
        ...
        axA.text(0.25,y0+0.07-i*0.22,title,ha="left",va="center",fontsize=13.9,fontweight="bold",color=INK)
        axA.text(0.74,y0+0.07-i*0.22,val,ha="right",va="center",fontsize=26,fontweight="bold",color=col)
    ...
    for i,(_,r) in enumerate(core.iterrows(),1):
        ...
        axB.text(0.20,y+0.025,clean_term(r["term"]).replace("\n"," "),ha="left",va="center",fontsize=14.6,fontweight="bold",color=INK)
        axB.text(0.20,y-0.035,f"OR {r['OR']:.2f}, {fmt_p(r['p_value'])}",ha="left",va="center",fontsize=12.8,color=SLATE)
        y-=0.155
    ...
    for i,rg in enumerate(row_order):
        for j,cg in enumerate(col_order):
            ...
            if len(r):
                ...
                axC.text(j,i,f"{rr.prev:.1f}%\n{int(rr.events)}/{int(rr.n)}",ha="center",va="center",fontsize=13.3,fontweight="bold",color=color)
    ...
    for i,(num,title,body) in enumerate(bullets):
        ...
        axD.text(0.21,y+0.018,title,ha="left",va="center",fontsize=14.2,fontweight="bold",color=INK)
        axD.text(0.21,y-0.032,body,ha="left",va="center",fontsize=14.2,color=SLATE)
        y-=0.15
    ...
    for t,b in title_options:
        axE.text(0.05,y,t,ha="left",va="center",fontsize=13.7,fontweight="bold",color=NAVY)
        axE.text(0.05,y-0.09,fill(b,44),ha="left",va="center",fontsize=14.2,color=INK,
...
def main():
    ...
    To avoid matplotlib memory/layout issues on some Windows installations,
    the default mode runs each figure in a fresh Python process. Use
    --figure 1 ... --figure 6 to generate one figure only.
    """
    ...
    if args.figure is None:
        for i in range(1, 7):
            print(f"Generating Figure {i} ...", flush=True)
            subprocess.run([sys.executable, str(Path(__file__).resolve()), "--figure", str(i)], check=True)
        print("Done. Outputs saved to:", OUT_DIR)
        return

    set_style()
    df, sheets = load_inputs()
    print(f"Loaded complete-case dataset: n={len(df)}, accidents={int(df['accident'].sum())}", flush=True)
    funcs = {
        1: figure1_discovery,
    ...
    funcs[args.figure](df, sheets)
    print(f"Figure {args.figure} complete", flush=True)
