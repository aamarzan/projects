...
Boss-level v2: fixed overlaps, premium per-panel palettes, larger panel gaps, EPS-safe.

Truth source:
  assay_table.csv (structured from thesis RFLP tables/results)

No invented results:
  all computations are deterministic transforms of assay_table fields.

Outputs: PNG (600 dpi) + PDF + EPS (EPS-safe; no transparency/alpha).
"""
...
def draw_gradient_bar(ax, c1, c2, steps=180):
    """EPS-safe gradient header (raster image; no alpha)."""
...
def panel_label(ax, letter):
    # inside placement prevents collisions with neighbors
...
def parse_fragments(cell):
    """Parse '300+150' or '74+95|169' -> list of alternatives."""
    if cell is None:
        return []
    s = str(cell).strip()
...
def signature_vector(row, k=3):
    """
    Vector for clustering/distance:
      [WT1..WTk, Mut1..Mutk, Amplicon, nWT, nMut]
    Uses first alternative for WT/Mut signatures.
    """
...
def robust_scale_0_1(X):
    """Robust column-wise scaling to [0,1] using median/MAD z-score."""
...
def locus_discriminator_score(row):
    """
    WT-vs-Mut separability score (higher is better):
      score = min(|WTband - Mutband|) + 20*|nMut - nWT|
    """
    wt  = first_or_empty(parse_fragments(row.get("wt_fragments_bp", "")))
    mut = first_or_empty(parse_fragments(row.get("mut_fragments_bp", "")))
...
def confusability_matrix(rows, tau=10.0):
    """
    Binary confusability between loci based on MUT signatures:
      confusable if same #bands AND max(|band_i - band_i'|) <= tau.
    """
...
def main():
    ...
    # impute missing with column medians for clustering
    X_imp = X.copy()
    ...
    # Heatmap uses fragments + amplicon only (distinct from Fig 3 lanes)
    heat_cols = list(range(0, 2*k)) + [2*k]
    H = Xo[:, heat_cols]

    # Per-panel colormaps (distinct premium sets)
    ...
    # ---------------- Layout: bigger gaps, dedicated cbar columns ----------------
    fig = plt.figure(figsize=(18, 11), dpi=170)
    fig.patch.set_facecolor(PAL["paper"])

    # BIGGER WSPACE/HSPACE eliminates your circled overlaps
    ...
    # -------- Panel B: dendrogram (premium single-color, not orange/green) --------
    tidy_axes(axB)
    indigo_hex = matplotlib.colors.to_hex(PAL["indigo"])
    dendrogram(
        Z, ax=axB, orientation="top", no_labels=True,
        color_threshold=0,                      # forces one-color scheme
    ...
    # -------- Panel A: heatmap (title now cannot collide with Panel B) --------
    tidy_axes(axA)
    imA = axA.imshow(H, aspect="auto", interpolation="nearest",
                     cmap=cmap_heat, vmin=0.0, vmax=1.0)
    panel_label(axA, "A")
    axA.text(0.5, 1.02, "Standardized fragment-size signature heatmap",
    ...
    # -------- Panel C: distance matrix (teal/mint palette) --------
    tidy_axes(axC)
    panel_label(axC, "C")
    ...
    # -------- Panel D: confusability (coral) + degree inset (teal) --------
    tidy_axes(axD)
    panel_label(axD, "D")
    ...
    inset.text(0.0, 1.02, "degree", transform=inset.transAxes,
               fontsize=8, color=PAL["slate"], ha="left")

    # -------- Panel E: top discriminators (gold) — y-labels won't touch Panel A now --------
    tidy_axes(axE)
    panel_label(axE, "E")
    topn = min(10, len(df_rank))
    show = df_rank.iloc[:topn].copy()

    axE.text(0.5, 1.02, "Top discriminators (WT vs Mut separation score)",
             transform=axE.transAxes, ha="center", va="bottom",
             fontsize=10.6, color=PAL["ink"])
    ...
    # IMPORTANT: save WITHOUT bbox_inches='tight' to avoid any post-layout squeezing/overlaps
    ...
    fig.savefig(str(base) + ".png", dpi=args.dpi, facecolor=PAL["paper"])
    fig.savefig(str(base) + ".pdf", facecolor=PAL["paper"])
    fig.savefig(str(base) + ".eps", format="eps", facecolor=PAL["paper"])
    plt.close(fig)
