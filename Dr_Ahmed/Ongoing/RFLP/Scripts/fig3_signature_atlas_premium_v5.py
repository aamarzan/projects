...
def make_figure(assay_csv: str, gel_path: str, outdir: str, title: str):
    ...
    draw_gradient_bar(ax_head, 0, 0, 1, 1, c1=PAL["violet"], c2=PAL["midnight"], steps=160)
    ...
    for g in genes:
        ...
        # move length label OUTSIDE right edge to avoid collisions
        axA.text(0.975, y, f"{gene_len[g]} aa", ha="right", va="center", fontsize=9, color=PAL["ink"])

        sub = df[df["gene"] == g]
        for _, r in sub.iterrows():
            pos = int(r["aa_pos_start"])
            x = 0.08 + 0.82 * (pos / gene_len[g])
        ...
        for frac, lab in [(0.0, "0"), (0.5, str(int(gene_len[g] * 0.5)))]:
            x = 0.08 + 0.82 * frac
        ...
        # end tick without numeric label (length already displayed)
        x_end = 0.90
